...
do
  FIRST_COLUMN=$(cat ../saida.dat | grep -i -A 9 n_${ALG}_$N | grep -i "$SEARCH" | head -n 1 | cut -d , -f 2)
  SECOND_COLUMN=$(cat ../saida.dat | grep -i -A 9 o_${ALG}_$N | grep -i "$SEARCH" | head -n 1 | cut -d , -f 2)
